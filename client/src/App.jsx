...
function App() {
  ...
  const filterData = (data, type) => {
    const now = dayjs(); //complete information of the present moment , the minute, the hour , the second , the week , the month, the year everything
    console.log(now)
  ...
  async function updateTask(taskId, newTaskText, addTime) {
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_BASE_URL}/api/tasks/${taskId}`, { taskText: newTaskText, addTime: addTime }, {
        headers: {
  ...
  async function deleteTask(taskId) {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/tasks/${taskId}`, {
        headers: {
          "Authorization": localStorage.getItem("token")
        }
      });
  ...
  async function addJournals(event) {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/add-task`, { taskText: task, timeSpent: time }, {
        headers: {
          "Content-Type": "application/json",
          "Authorization": localStorage.getItem("token")
        }
      ...
      // await fetchTasks()
      setTasks((prevTasksArr) => [...prevTasksArr,response.data] )
      fetchTasks();
